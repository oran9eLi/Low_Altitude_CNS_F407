...
- `Display`：HMI 显示预留层，提供显示模块入口和后续屏幕协议接入位置
...
- `Display`：预留 HMI 显示模块入口，后续由显示负责人接入页面变量和具体屏幕协议。
...
- `comm`：通信收发入口，后续接入LoRa、CAN 预留等模块
...
- 告警链路：模块统一构造 `App_AlarmMsg_t` 并通过 `App_AlarmPost()` 投递，告警任务按 `source + code + instance_id` 维护活动异常记录。
