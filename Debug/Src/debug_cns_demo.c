...
void DebugCnsDemo_RunOnce(void)
{
  ...
  read_result = Sensor_RegistryReadAll(&sample, 1U, &sample_count, &sensor_status, 1U, &status_count);
  ...
  if ((read_result == SENSOR_SEVERITY_NORMAL) &&
    ...
    DBG_PRINT("GNSS " DBG_TEXT_SAMPLE ": t=%lu " DBG_TEXT_LATITUDE "E7=%ld " DBG_TEXT_LONGITUDE "E7=%ld " DBG_TEXT_ALTITUDE "mm=%ld", (unsigned long)sample.timestamp_ms, (long)sample.value.geo.latitude_deg_e7, (long)sample.value.geo.longitude_deg_e7, (long)sample.value.geo.altitude_mm);

    DebugCnsDemo_BuildGnssPayload(&sample, payload);
    (void)App_ProtocolBuildFrame(DEBUG_CNS_FRAME_TYPE_GNSS, payload, DEBUG_CNS_GNSS_PAYLOAD_LEN, cns_frame, (uint16_t)sizeof(cns_frame), &cns_frame_len);

    s_frame_count++;
    DBG_PRINT("CNS " DBG_TEXT_FRAME " #%lu", (unsigned long)s_frame_count);
    DebugConsole_HexDump("CNS", cns_frame, cns_frame_len);
  } else {
    DBG_PRINT(DBG_TEXT_SENSOR_READ ": " DBG_TEXT_RESULT "=%u " DBG_TEXT_SAMPLE_COUNT "=%u " DBG_TEXT_STATUS "=%u", (unsigned int)read_result, (unsigned int)sample_count, (unsigned int)status_count);
  }
}
